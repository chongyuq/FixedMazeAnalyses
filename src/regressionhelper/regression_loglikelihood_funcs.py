...
def neg_log_likelihood(X, y, w):
    ...
    :param w: shape no. of features - this is to be optimized
    :type w: numpy array
    :return: negative log likelihood of observations given w
    :rtype:
    """
    y_pred = softmax(X @ w, axis=-1)
    negloglikelihood = - np.log(y_pred[range(len(y)), y]+1e-30).sum()
    return negloglikelihood


def neg_log_likelihood_with_fixed_constant(X, y, c, w):
    """
    ...
    """
    y_pred = softmax(X @ w + c, axis=-1)
    negloglikelihood = - np.log(y_pred[range(len(y)), y]+1e-30).mean()
    return negloglikelihood